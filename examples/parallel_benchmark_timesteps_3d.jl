...
function physics_301()
	...
	VoronoiFVM.Physics(; flux = g!, source = s)
	...
	VoronoiFVM.Physics(; reaction = reaction,
								   flux = flux,
								   source = source, 
								   storage = storage)
	=#
...
function example_time_normal3d(nm, dt, tpoints; verbose = false, unknown_storage = :sparse,
              method_linear = nothing, assembly = :cellwise,
              precon_linear = A -> VoronoiFVM.Identity(), do_init = true, do_plot = false, do_print_allocs=1, do_print_eaa=false, do_print_ts=true)
	...
	boundary_dirichlet!(sys, 1, 5, 0.0)
    boundary_dirichlet!(sys, 1, 6, 0.0)
    oldsol = unknowns(sys)
    oldsol .= 0.0
    ...
    control.reltol_linear = 0.0 #1.0e-5
...
function example_time_part_para3d(nm, dt, tpoints, nt; depth=2, verbose=false, unknown_storage=:sparse,
              method_linear = nothing, assembly = :cellwise,
              precon_linear = A -> VoronoiFVM.Identity(), do_init = true, do_plot = false, do_print_allocs=1, do_print_eaa=false, detail_allocs=false, do_print_ts=true)
...
function example_time_part_para_outside3d(nm, dt, tpoints, nt; depth=2, verbose=false, unknown_storage=:sparse,
              method_linear = nothing, assembly = :cellwise,
              precon_linear = A -> VoronoiFVM.Identity(), do_init = true, do_plot = false, do_print_allocs=1, do_print_eaa=false, detail_allocs=false, do_print_ts=true)
...
function validate_timesteps3d(nm, nt, depth, dt; tpoints=5, do_print_allocs=0, do_print_eaa=false, do_print_ts=false, do_part=true)
	@info "Computing normal solution"
	sols_normal    = example_time_normal3d(   nm, dt, tpoints;            do_plot=false, do_print_allocs, do_print_eaa, do_print_ts)
	
	@info "Computing parallelized solution"
	sols_part_para = example_time_part_para3d(nm, dt, tpoints, nt; depth, do_plot=false, do_print_allocs, do_print_eaa, do_print_ts, detail_allocs=false)
