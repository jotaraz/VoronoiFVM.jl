function eval_and_assemble_part_para_ESMP(
	...
	for level=1:depth
		@threads for tid=1:nt
			nodealloc[tid] += @allocated for item in cfp[(level-1)*nt+tid]
	...
	nodesepapre = 0
	@threads for tid=1:1
		nodesepaalloc = @allocated for item in cfp[depth*nt+1]
    		for inode in noderange(system.assembly_data, item)
	...
	for level=1:depth
		@threads for tid=1:nt
			edgealloc[tid] += @allocated for item in cfp[(level-1)*nt+tid]
    ...
    @threads for tid=1:1
		edgealloc[1] += @allocated for item in cfp[depth*nt+1]
			for iedge in edgerange(system.assembly_data, item)
	...
	nodealloc_sum = sum(nodealloc)
	edgealloc_sum = sum(edgealloc)
...
function eval_and_assemble_part_para_ESMP_outside(
    system::System{Tv,Tc,Ti,Tm,TSpecMat,TSolArray},
    U::AbstractMatrix{Tv}, # Actual solution iteration
    UOld::AbstractMatrix{Tv}, # Old timestep solution
    F::AbstractMatrix{Tv},# Right hand side
	...
	#nodealloc = zeros(Int64, nt)
	nodealloc = @allocated for level=1:depth
		@threads for tid=1:nt
			#nodealloc[tid] += @allocated 
	...
	nodesepaalloc = @allocated @threads for tid=1:1
		#nodesepaalloc = @allocated 
	...
	edgealloc = @allocated for level=1:depth
		@threads for tid=1:nt
			#edgealloc[tid] += @allocated 
			for item in cfp[(level-1)*nt+tid]
				for iedge in edgerange(system.assembly_data, item)
					_fill!(edges[tid], system.assembly_data, iedge, item)
    ...
    edgealloc += @allocated @threads for tid=1:1
		#edgealloc[1] += @allocated 
		for item in cfp[depth*nt+1]
			for iedge in edgerange(system.assembly_data, item)
				_fill!(edges[1], system.assembly_data, iedge, item)
	...
	nodealloc_sum = (nodealloc)
	edgealloc_sum = (edgealloc)
