...
function node_asm_count!(
	...
	for item in cells
		for inode in noderange(system.assembly_data, item)
			...
			allocs[2] += @allocated assemble_res_jac_reordered(node, system, asm_res, asm_jac, asm_param, new_ind)
		end
	end
	allocs
end
...
function edge_asm_count!(
	...
	for item in cells
		for iedge in edgerange(system.assembly_data, item)
			...
			allocs[2] += @allocated assemble_res_jac_reordered(edge, system, asm_res, asm_jac, asm_param, new_ind)
			...
			if isnontrivial(erea_evaluator)
			    ...
			    allocs[2] += @allocated assemble_res_jac_reordered(edge, system, ereaasm_res, ereaasm_jac, ereaasm_param, new_ind)
			end
			...
			if isnontrivial(outflow_evaluator) && hasoutflownode(edge)
			    ...
			    allocs[2] += @allocated assemble_res_jac_reordered(
			        edge,
			        system,
			        outflowasm_res,
			        outflowasm_jac,
